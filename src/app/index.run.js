  ...
  function runBlock($log, $window, $rootScope, $timeout, Util, /*MODULES,*/ SCENE_WIDTH, SCENE_HEIGHT) {
    ...
    /*$rootScope.$on('$stateChangeSuccess', function(){
      $timeout(function(){
        ...
        var p_elements = angular.element('p');
        for(var i = 0; i < p_elements.length; i++){
          var p_el = angular.element(p_elements[i]);
          if(p_el.hasClass('vertical')){
            p_el.css('transition', 'opacity 0.5s');
            p_el.css('opacity', 1);
          }else{
            var delayTime = Math.min((i/10), 1.5);
            p_el.css('animation-delay', delayTime + 's');
            p_el.addClass('animate animate-top')
          }
        }
      },500)
    });*/
